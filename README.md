...
### Инструкция: Как запустить проект через Docker + Docker Compose в 2 шага перейдите по ссылке:
https://github.com/Andrey-G1Thub/bookez-hotel-booking/tree/docker-deploy

После этого проект будет полностью доступен в браузере по адресу: http://localhost


### Инструкция: Как запустить проект через Docker следуйте за инструкцией:
